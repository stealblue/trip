...
const PageNavComp = ({ totalCount, pageNo, numOfRows }) => {
  ...
  let pageNavEndNum = Math.ceil(page / numOfRows) * numOfRows;
  pageNavEndNum = res <= pageNavEndNum ? res : Math.ceil(page / numOfRows) * numOfRows; // 현재 페이지가 속하는 페이지 네비게이션의 시작 페이지
  let pageNavStartNum = Math.floor(page / numOfRows) * numOfRows; // 현재 페이지가 속하는 페이지 네비게이션의 마지막 페이지
  ...
  return (
    <Pagination>
      <p>
        {pageNavStartNum !== 1 && pageNavStartNum > numOfRows ? (
          <li onClick={onClickTest} value={pageNavStartNum - 1} data-page={pageNavStartNum - 1}>
            prev
          </li>
        ) : null}
        {resArray.map((r) => (
          <LiBlock onClick={onClickTest} key={r} value={r}>
            {r}
          </LiBlock>
        ))}
        {pageNavEndNum < res ? (
          <li onClick={onClickTest} value={pageNavEndNum + 1} data-page={pageNavEndNum + 1}>
            next
          </li>
        ) : null}
      </p>
